...
interface BookingPaymentsTabProps {
  booking: {
    id: string;
    status: string;
...
export function BookingPaymentsTab({
  ...
  const texts = {
    vi: {
      ...
      latePaymentWarning: 'Đây là booking khách hàng chuyển khoản trễ lúc đặt',
    },
    en: {
      ...
      latePaymentWarning: 'This booking was paid late after the payment deadline',
    },
  };
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/admin/bookings/${booking.id}/payments`);
        if (!response.ok) {
  ...
  const handleAddPayment = async (data: { amount: number; paymentMethod: string; notes: string }) => {
    ...
    try {
      const response = await fetch(`/api/admin/bookings/${booking.id}/add-payment`, {
        method: 'POST',
      ...
      const paymentsResponse = await fetch(`/api/admin/bookings/${booking.id}/payments`);
  ...
  const handleEditPayment = async (data: { amount: number; paymentMethod: string; notes: string }) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/bookings/${booking.id}/payments/${selectedPayment.id}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-4">
      ...
      {booking.hasLatePayment && (
        <div className="flex items-center gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg text-orange-800">
          ...
          <span className="text-sm font-medium">{t.latePaymentWarning}</span>
        </div>
      )}
