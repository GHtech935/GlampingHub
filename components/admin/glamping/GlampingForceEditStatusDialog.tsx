...
// Glamping status types - aligned with camping
type GlampingBookingStatus = 'pending' | 'confirmed' | 'checked_in' | 'checked_out' | 'cancelled';
type GlampingPaymentStatus = 'pending' | 'deposit_paid' | 'fully_paid' | 'refund_pending' | 'refunded' | 'no_refund' | 'expired';

interface GlampingForceEditStatusDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (status: GlampingBookingStatus, paymentStatus: GlampingPaymentStatus) => void;
  currentStatus: GlampingBookingStatus;
  currentPaymentStatus: GlampingPaymentStatus;
  isLoading: boolean;
  locale: string;
}

const STATUS_OPTIONS: { value: GlampingBookingStatus; labelVi: string; labelEn: string }[] = [
...
const PAYMENT_STATUS_OPTIONS: { value: GlampingPaymentStatus; labelVi: string; labelEn: string }[] = [
...
export function GlampingForceEditStatusDialog({
  isOpen,
  onClose,
  onConfirm,
  currentStatus,
  ...
  isLoading,
  locale,
}: GlampingForceEditStatusDialogProps) {
  const [status, setStatus] = useState<GlampingBookingStatus>(currentStatus);
  const [paymentStatus, setPaymentStatus] = useState<GlampingPaymentStatus>(currentPaymentStatus);
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={status} onValueChange={(v) => setStatus(v as GlampingBookingStatus)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select value={paymentStatus} onValueChange={(v) => setPaymentStatus(v as GlampingPaymentStatus)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
