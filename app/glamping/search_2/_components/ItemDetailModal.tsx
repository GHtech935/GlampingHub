      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-0">
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex justify-end gap-3 p-6 border-t bg-gray-50">
            ...
            <Button
              onClick={() => {
                // Build booking form URL with params
                const params = new URLSearchParams();
                params.set('itemId', item.id);
                params.set('itemName', item.name);
                params.set('zoneName', zoneName || 'Glamping Zone');

                if (dateRange.checkIn) {
                  const checkIn = `${dateRange.checkIn.getFullYear()}-${String(dateRange.checkIn.getMonth() + 1).padStart(2, '0')}-${String(dateRange.checkIn.getDate()).padStart(2, '0')}`;
                  params.set('checkIn', checkIn);
                }

                if (dateRange.checkOut) {
                  const checkOut = `${dateRange.checkOut.getFullYear()}-${String(dateRange.checkOut.getMonth() + 1).padStart(2, '0')}-${String(dateRange.checkOut.getDate()).padStart(2, '0')}`;
                  params.set('checkOut', checkOut);
                }

                // Get adults and children from current URL params or default
                ...
                if (item.pricing?.base_price) {
                  params.set('basePrice', item.pricing.base_price.toString());
                }

                // Redirect to booking form
                router.push(`/glamping/booking/form?${params.toString()}`);
              }}
