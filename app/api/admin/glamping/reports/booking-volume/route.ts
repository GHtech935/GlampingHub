...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Date range filter on booking created_at
    const dateCol = "b.created_at";
    ...
    // Group by month, aggregate booking counts + tent-nights + revenue
    const dataQuery = `
      SELECT
        TO_CHAR(b.created_at, 'YYYY-MM') as month,
        TO_CHAR(b.created_at, 'Mon YYYY') as month_label,
        COUNT(DISTINCT b.id) as bookings,
        COUNT(bt.id) as qty,
        COALESCE(SUM(DISTINCT b.total_amount), 0) as total
      ${fromClause}
      ${whereClause}
      GROUP BY TO_CHAR(b.created_at, 'YYYY-MM'), TO_CHAR(b.created_at, 'Mon YYYY')
      ORDER BY month ASC
    `;
    ...
    console.error("Error fetching booking volume:", error);
    return NextResponse.json({ error: "Failed to fetch booking volume data" }, { status: 500 });
  } finally {
    client.release();
  }
}
