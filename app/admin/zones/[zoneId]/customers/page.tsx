...
export default function ZoneCustomersPage({ params }: { params: Promise<{ zoneId: string }> }) {
  ...
  const fetchCustomers = async () => {
    if (zoneId === "all") return; // Skip fetching if all zones

    setLoading(true);
    try {
      const params = new URLSearchParams();
      if (search) params.append('search', search);
      params.append('zoneId', zoneId); // Add zoneId parameter
    ...
    try {
      ...
      const csvRows = [
        ...
        ...customers.map(c => [
          ...
          c.total_bookings,
          c.total_spent,
          c.last_booking_date ? new Date(c.last_booking_date).toLocaleDateString('vi-VN') : '',
      ...
      link.setAttribute('download', `customers_zone_${zoneId}_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="p-0 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">{t('total')} {t('customers')}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">{t('vip')}</p>
                <p className="text-2xl font-bold text-gray-900">
                  {customers.filter(c => c.total_spent >= 50000000).length}
                </p>
              </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">{t('bookings')}</p>
                <p className="text-2xl font-bold text-gray-900">
      ...
      <Dialog open={tierInfoOpen} onOpenChange={setTierInfoOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl">{t('tierSystem.title')}</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-gray-900">≥ 50,000,000 VND</p>
                  <p className="text-sm text-gray-600">{t('tierSystem.vipDesc')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-gray-900">≥ 20,000,000 VND</p>
                  <p className="text-sm text-gray-600">{t('tierSystem.goldDesc')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{t('tierSystem.silverDesc')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-orange-50 rounded-lg border border-orange-200">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-gray-900">&lt; 5,000,000 VND</p>
                  <p className="text-sm text-gray-600">{t('tierSystem.bronzeDesc')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              ...
              <ul className="text-sm text-blue-800 space-y-1 ml-6 list-disc">
                <li>Tier được tính dựa trên tổng số tiền từ tất cả các booking</li>
                <li>Chỉ tính các booking có trạng thái <strong>không phải &ldquo;Đã hủy&rdquo;</strong></li>
                <li>Tier được cập nhật tự động khi khách hàng có booking mới</li>
              </ul>
            </div>
          </div>
        </DialogContent>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('deleteWarning').replace('{name}', customerToDelete?.name || '')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
