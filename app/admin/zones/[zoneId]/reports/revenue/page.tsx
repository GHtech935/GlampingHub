...
export default function RevenuePage() {
  ...
  const t = useTranslations("admin.reportRevenue");
  const tFilters = useTranslations("admin.reportRevenue.filters");
  const tCols = useTranslations("admin.reportRevenue.columns");
  const tActions = useTranslations("admin.reportRevenue.actions");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/glamping/reports/revenue?${p}`);
  ...
  const columns: ColumnDef<RevenueRow>[] = useMemo(() => [
    { key: "monthLabel", header: tCols("period") },
    { key: "transactions", header: tCols("bookingCount"), align: "center" },
    {
      key: "payments",
      header: tCols("paymentsReceived"),
      align: "right",
      render: (r) => <span className="text-green-700">{fmtCur(r.payments)}</span>,
    },
    {
      key: "refunds",
      header: tCols("refunds"),
      align: "right",
      render: (r) => <span className="text-red-600">{fmtCur(r.refunds)}</span>,
    },
    {
      key: "total",
      header: tCols("netRevenue"),
      align: "right",
  ...
  const handleExportExcel = async () => {
    ...
    await exportToExcel(exportData, exportCols, { title: t("title"), filename: "revenue-report" });
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(exportData, exportCols, { title: t("title"), filename: "revenue-report" });
