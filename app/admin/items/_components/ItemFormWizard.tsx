...
import { ArrowLeft, ArrowRight, Save, Eye, Upload, X, Image as ImageIcon, ChevronDown, HelpCircle, Sun, Moon, Clock, Calendar, Check, Paperclip, Loader2 } from "lucide-react";
...
const formSchema = z.object({
  ...
  // Allocation settings
  fixed_length_value: z.number().optional(),
  fixed_length_unit: z.enum(['days', 'nights', 'hours']).optional(),
  fixed_start_time: z.string().optional(),
  default_length_hours: z.number().optional(),
});
...
const getSteps = (t: any) => [
  ...
  { id: 5, name: t('steps.packages'), key: 'packages' },
];
...
export function ItemFormWizard({
  ...
  useEffect(() => {
    if (mode === 'edit' && initialData) {
      ...
      // TODO: Remove event_pricing if no longer used
      ...
      if (initialData.package_items) {
        setPackageItems(initialData.package_items);
      }
      if (initialData.show_package_price !== undefined) {
        setShowPackagePrice(initialData.show_package_price);
      }
      if (initialData.package_starting_price !== undefined) {
        setPackageStartingPrice(initialData.package_starting_price);
      }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Conditional API endpoint and method based on mode
      const url = mode === 'create'
        ? '/api/admin/glamping/items'
        : `/api/admin/glamping/items/${itemId}`;
      ...
      if (!response.ok) {
        throw new Error(result.error || (mode === 'create' ? 'Failed to create item' : 'Failed to update item'));
      }

      // Get the current item ID for event attachment
      const currentItemId = mode === 'create'
      ...
      toast({
        title: "Thành công",
        description: mode === 'create' ? "Tạo item thành công" : "Cập nhật item thành công",
      });
      ...
      if (onSuccess && finalItemId) {
        ...
        router.push(mode === 'create' ? '/admin/items' : `/admin/items/${finalItemId}`);
      }
    } catch (error: any) {
      console.error(mode === 'create' ? 'Create item error:' : 'Update item error:', error);
      toast({
        title: "Lỗi",
        description: error.message || (mode === 'create' ? "Không thể tạo item" : "Không thể cập nhật item"),
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleNext = async () => {
    // If leaving step 1, create pending categories in DB first
    // Tags will be created during final form submission
    if (currentStep === 1 && pendingCategories.length > 0) {
      try {
        setLoading(true);
        ...
        // Clear pending categories only (keep tags pending until final submit)
        setPendingCategories([]);
  ...
  const handleCreateParameter = async (formData: any) => {
    ...
    try {
      ...
      const parametersRes = await fetch('/api/admin/glamping/parameters');
  ...
  const handleAttachExisting = async () => {
    try {
      // Fetch all available events from API
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-between  mx-auto">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {mode === 'create' ? t('title') : 'Edit Item'} - {STEPS[currentStep - 1].name}
            </h1>
            <p className="text-sm text-gray-600 mt-1">
              {currentStep === 1 && (mode === 'create' ? t('stepDesc.description') : 'Update item description and details')}
              {currentStep === 2 && (mode === 'create' ? t('stepDesc.media') : 'Update images and videos')}
              {currentStep === 3 && (mode === 'create' ? t('stepDesc.attributes') : 'Update attributes and inventory settings')}
              {currentStep === 4 && (mode === 'create' ? t('stepDesc.pricing') : 'Update pricing configuration')}
              {currentStep === 5 && (mode === 'create' ? t('stepDesc.packages') : 'Update packages and add-ons')}
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      {selectedTags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-3 p-3 border rounded-lg bg-gray-50">
                        ...
                        <FormItem>
                          ...
                          {field.value && (
                            <div className="text-sm text-gray-600 mt-1">
                              ...
                              <button type="button" className="text-blue-600 hover:underline">{t('edit')}</button>
                            </div>
                          )}
                          <FormMessage />
                        </FormItem>
                          ...
                          <table className="w-full">
                            ...
                            <tbody>
                              ...
                              {attachedParameters.filter(p => p.inventory === 'Controlled').map((param) => (
                                <tr key={param.id} className="border-b last:border-0 hover:bg-gray-50">
                                  ...
                                  <td className="px-4 py-3 text-sm text-gray-600">{param.inventory}</td>
                                  <td className="px-4 py-3 text-sm text-gray-600">{param.visibility}</td>
                                  <td className="px-4 py-3">
                                    <div className="flex items-center gap-2">
                                      <Input
                                        type="number"
                                  ...
                                  <td className="px-4 py-3">
                                    ...
                                    >
                                      ...
                                      <SelectContent>
                                        ...
                                        <SelectItem value="remove">{t('removeAction')}</SelectItem>
                                      </SelectContent>
                                    </Select>
                                  </td>
                                </tr>
                      ...
                      <div className="flex gap-2 mt-4">
                        <DropdownMenu open={showCreateEventDropdown} onOpenChange={setShowCreateEventDropdown}>
                          ...
                          <DropdownMenuContent align="start" className="w-80">
                            ...
                            <DropdownMenuItem onClick={() => handleEventTypeSelect('exclusive_date_span')}>
                              <Clock className="w-4 h-4 mr-2 text-purple-500" />
                              <div>
                                <div className="font-medium">{t('itemEvents.types.exclusiveDateSpan')}</div>
                                <div className="text-xs text-gray-500">{t('itemEvents.types.exclusiveDateSpanDesc')}</div>
                              </div>
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                    ...
                    <div className="mt-8">
                      ...
                      <div>
                        ...
                        <Select value={depositType} onValueChange={setDepositType}>
                          ...
                          <SelectContent>
                            <SelectItem value="system_default">Use system settings (50%)</SelectItem>
                            <SelectItem value="custom_percentage">% Percent</SelectItem>
                            <SelectItem value="fixed_amount">đ VND (Fixed amount)</SelectItem>
                            <SelectItem value="per_hour">đ VND (một hour)</SelectItem>
                            <SelectItem value="per_qty">đ VND (per qty)</SelectItem>
                    ...
                    <div>
                                ...
                                <tr key={index} className="border-b last:border-0">
                                  ...
                                  <td className="px-4 py-3">
                                    <Input
                                      ...
                                      min="0"
                                      max="100"
                                      value={item.price_percentage}
                                      onChange={(e) => {
                                        const newItems = [...packageItems];
                                        newItems[index].price_percentage = parseInt(e.target.value) || 0;
                      ...
                      <div className="mt-4">
                        <Button
                          type="button"
                          onClick={() => {
                            // Fetch available items when opening dialog
                            fetch('/api/admin/glamping/items')
                              .then(res => res.json())
                              .then(data => {
                    ...
                    <div className="border-t pt-6">
                      <div className="flex items-center justify-between mb-4">
                        <label className="text-sm font-medium text-gray-700">
                          {t('packages.showStartingPrice')}
                        </label>
                        <Button
                          type="button"
                          variant={showPackagePrice ? "default" : "outline"}
                          size="sm"
                          onClick={() => setShowPackagePrice(!showPackagePrice)}
                        >
                          {showPackagePrice ? t('on') : t('off')}
                        </Button>
                      </div>
                      ...
                      {showPackagePrice && (
                        <div className="mt-4">
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            {t('packages.customStartingPrice')}
                          </label>
                          <Input
                            type="number"
                            min="0"
                            value={packageStartingPrice}
                            onChange={(e) => setPackageStartingPrice(parseInt(e.target.value) || 0)}
                            className="w-64"
                            placeholder={t('packages.enterCustomPrice')}
                          />
                          <p className="text-sm text-muted-foreground mt-1">
                            {t('packages.overridePriceHint')}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <div className="bg-gray-50 rounded-lg p-6 sticky top-6">
                  ...
                  {currentStep === 5 && (
                    <>
                      ...
                      <p className="text-sm text-gray-500 italic mb-6">
                        {t('sidebar.addOnsExamples').split('\n').map((line, i) => (
                          <span key={i}>
                            {line}
                            {i < t('sidebar.addOnsExamples').split('\n').length - 1 && <br />}
                          </span>
                        ))}
                      </p>

                      <h3 className="font-semibold text-gray-900 mb-2">{t('sidebar.pricePercentageTitle')}</h3>
                      <p className="text-sm text-gray-600 mb-4">
                        {t('sidebar.pricePercentageDesc')}
                      </p>
                      <p className="text-sm text-gray-500 mb-6">
                        {t('sidebar.pricePercentageExamples').split('\n').map((line, i) => (
                          <span key={i}>
                            {line}
                            {i < t('sidebar.pricePercentageExamples').split('\n').length - 1 && <br />}
                          </span>
                        ))}
                      </p>

                      <h3 className="font-semibold text-gray-900 mb-2">{t('sidebar.packageStartingPriceTitle')}</h3>
                      <p className="text-sm text-gray-600 mb-4">
                        {t('sidebar.packageStartingPriceDesc')}
                      </p>
                      <p className="text-sm text-gray-500 italic">
                        {t('sidebar.packageStartingPriceExample')}
                      </p>
                    </>
                  )}
      ...
      <Dialog open={showCreateParameterModal} onOpenChange={setShowCreateParameterModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <ParameterForm
            ...
            onCancel={() => setShowCreateParameterModal(false)}
                  ...
                  availableEvents.map((event) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-gray-600">{event.start_date || '-'}</td>
                        <td className="py-3 px-4 text-gray-600">{event.end_date || '-'}</td>
                        <td className="py-3 px-4">
