...
export default function EditTagPage() {
  ...
  const fetchTag = async () => {
    try {
      const response = await fetch(`/api/admin/glamping/tags/${params.id}`);
      ...
      setFormData({
        name: data.tag.name,
        weight: data.tag.weight,
        visibility: data.tag.visibility,
      });
    } catch (error: any) {
      toast({
        title: "Error",
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-sm text-gray-500">
                Tags with higher weight appear first in lists
              </p>
            </div>
